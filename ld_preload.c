...
void *syscall_thread()
{
    ...
    DEBUG_SYSTHREAD("SYSCALL_THREAD() : syscall thread created, tid = \n",pthread_self())
        ...
        while(send_data.request_type != TYPE_CLOSE && send_data.request_type != TYPE_SHUTDOWN){
            ...
            switch(send_data.request_type){
                ...
                case TYPE_SOCKET:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : socket() called by syscall thread\n")
                ...
                case TYPE_BIND:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : bind() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                ...
                case TYPE_LISTEN:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : listen() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                ...
                case TYPE_ACCEPT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : accept() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_CONNECT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : connect() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_SEND:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : send() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_RECV:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : recv() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_SETSOCKOPT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : setsockopt() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_GETSOCKOPT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : getsockopt() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_READ:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : read() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_WRITE:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : write() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                ...
                case TYPE_POLL:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : poll() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_PPOLL:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : ppoll() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_EPOLL_WAIT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : epoll_wait() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_EPOLL_CREATE:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : epoll_create() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_EPOLL_CTL:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : epoll_ctl() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_SOCKETPAIR:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : socketpair() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_SENDTO:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : sendto() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_RECVFROM:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : recvfrom() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_SENDMSG:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : sendmsg() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_RECVMSG:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : recvmsg() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_SHUTDOWN:
...
//                    else errno = old_errno;                         // errno not changed
                ...
                case TYPE_GETSOCKNAME:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : getsockname() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_GETPEERNAME:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : getpeername() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_OPEN:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : open() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_OPENAT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : openat() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                ...
                case TYPE_LSEEK:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : lseek() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_LSEEK64:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : lseek64() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_STAT:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : stat() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                case TYPE_STAT64:
                    ...
                    DEBUG_SYSTHREAD("SYSCALL_THREAD() : stat64() called by syscall thread\n")
                    if(errno != 0) recv_data.thr_errno = errno;     // set changed errno
                    else errno = old_errno;                         // errno not changed
                    break;
                default:
